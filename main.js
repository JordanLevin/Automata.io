...
function load_automata(str) {
    ...
    for (let node of temp.nodes) {
        ...
        for (let t of node.transitions) {
            let newtran =
                new Transition(t.x1, t.y1, t.x2, t.y2, t.dst, t.chars);
...
function canvasClick(event) {
    ...
    left_clicked(
        canvas_to_viewX(mouseX),
        canvas_to_viewY(mouseY));
...
function mouseDragged() {
    mouse_dragged(mouseX/g_scale, mouseY/g_scale);
    return false;
}
function mouseWheel(event) {
    if (event.delta < 0)
        g_scale = Math.max(g_scale + event.delta / 50, .25);
    else
        g_scale = Math.min(g_scale + event.delta / 50, 3);
    return false;
}
